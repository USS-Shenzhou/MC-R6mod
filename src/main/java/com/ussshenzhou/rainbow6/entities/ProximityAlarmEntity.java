...
public class ProximityAlarmEntity extends ProjectileItemEntity {
    ...
    public void tick() {
        ...
        if (this.onGround) {
            ...
            int i = world.getLightFor(LightType.SKY, this.getPosition()) - world.getSkylightSubtracted();
            float f = world.getCelestialAngleRadians(1.0F);
            float f1 = f < (float) Math.PI ? 0.0F : ((float) Math.PI * 2F);
            f = f + (f1 - f) * 0.2F;
            i = Math.round((float) i * MathHelper.cos(f));
            i = MathHelper.clamp(i, 0, 15);
            this.light = 16 * Math.max(i, world.getLightFor(LightType.BLOCK, this.getPosition().offset(this.dataManager.get(direction))));
